...
const App = () => {
  ...
  useEffect(() => {
    //alert('Pixelly is on maintenance mode. Please do not use it for now. We will be back soon. Thank you for your patience.');
    const fetch = () => {
      axios
        .get(
          'https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=zookeeper&order=market_cap_desc&per_page=100&page=1&sparkline=false'
        )
        .then(response => {
          if (response?.data?.[0])
        ...
        if (chainId === 888) {
          ...
          const oracle = new ethers.Contract(
            ...
            [
              {
                inputs: [
                  { internalType: 'address', type: 'address', name: '_token' },
                ],
                name: 'getPrice',
