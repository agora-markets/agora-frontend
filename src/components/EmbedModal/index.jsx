...
const EmbedModal = ({ visible, onClose, embedTitle }) => {
  ...
  const frameProps = {
    ...
    src:
      ...
      '?' +
      ['embed', darkMode ? 'theme=dark' : ''].filter(x => !!x).join('&'),
    title: ['Pixelly', embedTitle].filter(x => !!x).join(' - '),
  };
  ...
  return (
    <Modal visible={visible} onClose={onClose} title="Embed Collection">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        <textarea style={{ height: 160 }} readOnly value={value} />
        <div
          style={{
            display: 'flex',
            alignItems: 'center',
